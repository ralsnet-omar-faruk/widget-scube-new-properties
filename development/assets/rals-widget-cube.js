(function () {
  ...
  async function renderWidget(container) {
    try {
      ...
      // Support both card format and list format
      let template = container.querySelector(".property-card");
      ...
      const cardsHTML = properties
        .map((property) => {
          ...
          // Handle date - for list format
          const dateEl = card.querySelector(".property-date");
          if (dateEl) {
          ...
          // Handle comment - for list format
          ...
          // Handle property-link (anchor wrapping the whole item)
